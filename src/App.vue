...
// 할일 완료된(completed가 true인) 요소의 배열 길이와
// 완료된 요소를 새로운 배열로 만들고 그 갯수를 구해줌
...
// 할일 추가
// 할일 객체데이터는 고유 id를 추가하여 데어트를 구분할 수 있도록 해야함
function addItem() {
  data.items.push({
  ...
  // 할일 추가한 후 입력필드와 연결된 데이터를 빈칸으로 초기화
  data.newItem = '';
}
// 할일 삭제
...
<template>
  <main class="app">
    ...
    <ul class="todo_list">
      ...
      <!-- 리스트는 고유 값을 key속성에 연결하여 순서가 변경돼도 구별가능 하도록 해야함 -->
      <!-- todo.completedrk true면 completed라는 문자열이 클래명으로 추가됨 -->
      <li
        v-for="(todo, i) in data.items"
        v-bind:key="todo.id"
        v-bind:class="{ completed: todo.completed }"
      >
        <!-- id가 달라야되므로 id속성과 todo.id값을 단방향으로 연결 -->
        <input
          type="checkbox"
